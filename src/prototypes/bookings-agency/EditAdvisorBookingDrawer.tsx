 ...
 * so the dropdown opens already pointing at the right option. Falls back
 * to the first split if there's no exact match.
 */
function splitIdForPercentage(percentage: number): string {
  return initialSplits.find((s) => s.percentage === percentage)?.id ?? initialSplits[0]?.id ?? ''
}
...
export function EditAdvisorBookingDrawer({ open, booking, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    ...
    setCurrency('USD')
    setSplitId(splitIdForPercentage(booking.split))
    setNotes('')
  }, [open, booking])
  ...
  const splitPct = selectedSplit?.percentage ?? 0
