...
function RowMenu({ campaign, onEdit, onViewReport }: { campaign: Campaign; onEdit: () => void; onViewReport: () => void }) {
        ...
        <>
          ...
          <div className="absolute right-0 top-8 z-20 bg-white border border-travefy-gray-200 rounded-lg shadow-lg py-1 w-40 text-sm">
            {campaign.status === 'sent' && (
              <button onClick={() => { onViewReport(); setOpen(false) }} className="w-full px-3 py-2 text-left hover:bg-travefy-gray-50 text-travefy-gray-700">View Report</button>
            )}
...
export function CampaignsTab() {
  ...
  return (
    <div className="p-5 space-y-5">
      {/* Stats */}
      <div>
        <p className="text-xs font-semibold text-travefy-gray-500 uppercase tracking-wide mb-3">Last 30 days</p>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<Mail className="w-4 h-4" />} label="Total Campaigns" value={String(campaigns.length)} />
        <StatCard icon={<Send className="w-4 h-4" />} label="Emails Sent" value={totalSent.toLocaleString()} />
        <StatCard icon={<BarChart2 className="w-4 h-4" />} label="Avg Open Rate" value={avgOpenRate} />
        <StatCard icon={<MousePointerClick className="w-4 h-4" />} label="Avg Click Rate" value={avgClickRate} />
      </div>
      </div>
